# Dadabe v0.6.2 — Song Arrangement + Reharmonisation

## Overview

v0.6 (`docs/v0.6/design.md`) shipped §1–§3: the strictness policy (D54), the out-of-key
chord policy (D46), and the key-aware transform family (`diatonic-transpose`,
`parallel-mode`, `substitute`, `negative-harmony`). This document picks up everything else
v0.6 held from `docs/v0.5.2/ui-song-mode.md` and `docs/v0.5.2/progression-transforms.md` —
renumbered from v0.6's old §4–§16 to §1–§13 here, with no other changes to scope or
decisions. (`docs/v0.6.1/` is unrelated — bug notes and prompt scratch files, not a design
continuation.)

Tags carried over from v0.6:

- **[KEY]** — depends on `KeyInference` succeeding, with the degradation path v0.6 §2
  established (a uniform "needs a key" error plus a key-override control) rather than
  falling back to C major.
- **[DEP]** — depends on v0.6 shipping first, or on another item in this release.
- **[EXP]** — genuinely expensive: a new solver mode, a new entity, or a cross-cutting
  display change.

All open questions from the source specs were resolved while drafting v0.6 and carry
forward unchanged; nothing here is blocked on a decision, only on other work in this
release.

Decisions continue from D54 (the last one used in v0.6). New decisions here are numbered
D57+.

---

## 1. Length-changing transforms (D52) — [KEY]

Every transform shipped so far (v0.5.2's eleven key-blind transforms, v0.6's four key-aware
ones) preserves chord count. The two most common reharmonisations a working musician
performs do not, which is the biggest single gap in the source taxonomy.
...
run backwards. (Note: `IsValidNonDiatonic` is `private` in `KeyInference.cs` as of v0.6 —
this section needs it exposed, or its rule extracted, the same way v0.6 exposed
`MajorIntervals`/`MinorIntervals` as `internal` for `ScaleTriads`/`DiatonicModes`.)
...
- **Song slots** carry `bars`. Inserting a chord has to split a slot's bars or push the
  ...
  form (§2) and loop closure (§3) math stable.
...
v0.6's key-aware transforms (D47) shipped first, as planned; this section can proceed
without further prerequisites of its own.

---

## 2. Song arrangement / form (D40) — [DEP] — Resolved
...
## 3. Loop closure (D41) — [EXP, DEP on §2]
...
## 4. Song transposition and capo (§5.7) — [DEP on v0.5.2 §1–§2]
...
## 5. Retune flow (D55) — [DEP] — Resolved
...
Shared by §4's transpose, which has the same invalidate-and-refill shape. Build it once.

## 6. Slot-inline voicing search (D56) — Resolved
...
## 7. Songwriting loop (§5.8) — [DEP on v0.6 §3]
...
Depends on v0.6 §3 (shipped) only for the key-override path — prediction already accepts
context, so the playability filter and auto-pin are the new work.

## 8. Analysis read-outs — [KEY]
...
## 9. Saved transform chains (D50, remainder) — [DEP on v0.5.2 §3]
...
## 10. Transform this section — [DEP on §2, v0.5.2 §3]
...
length-changing transforms (§1) settled first, because that is where slot `bars` handling
gets decided.
...
1. **§2 song form** — unblocks §3 and §10, and nothing here depends on it.
2. **§5 retune flow**, then **§4 transposition and capo**, which reuses it.
3. **§6 slot-inline search** — decide the workflow question first; it changes where voicing
   search lives.
4. **§1 length-changing transforms** — after §2 settles slot `bars`.
5. **§3 loop closure**, **§7 songwriting loop**, **§8 analysis**, **§9 saved chains** —
   independent of each other, order by appetite.
6. **§11 setlists** — lowest priority; ship whenever there's appetite for a new entity.
